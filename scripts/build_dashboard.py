...
def line_svg(frame: pd.DataFrame, width: int = 960, height: int = 300) -> str:
    ...
    def points(series):
        return " ".join(
            f"{pad + i * (width - 2 * pad) / (len(series) - 1):.1f},{height - pad - (value - lo) * (height - 2 * pad) / (hi - lo):.1f}"
    ...
    return f'''<svg viewBox="0 0 {width} {height}" role="img" aria-label="Actual and predicted daily peak load">
      ...
      <text x="{pad}" y="18" fill="#627084" font-size="12">{hi:,.0f} MW</text>
      <text x="{pad}" y="{height-8}" fill="#627084" font-size="12">{lo:,.0f} MW</text>
    </svg>'''
...
<section class="note"><b>Interpretation boundary:</b> The load observations are official ISO-NE data. Weather is a six-city regional average. Battery economics are transparent illustrative assumptions, not an MMWEC tariff or realized savings claim.</section>
