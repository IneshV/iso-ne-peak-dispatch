...
def main() -> None:
    ...
    weather = load_or_download_weather(
        ROOT / "data" / "raw" / "weather" / "new_england_hourly_weather.csv",
    ...
    _, test, results = train_and_evaluate(daily, artifacts)
    ...
    dispatch, summary = backtest_monthly_dispatch(test)
    dispatch.to_csv(artifacts / "dispatch_backtest_2025.csv", index=False)
    (artifacts / "dispatch_summary.json").write_text(json.dumps(summary, indent=2) + "\n")
